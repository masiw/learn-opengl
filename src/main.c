...
static void glDebugOutput_callback(GLenum source,
                                   GLenum type,
                                   unsigned int id,
                                   GLenum severity,
                                   GLsizei length,
...
int main(void) {
  ...
  // Restrict context version
  glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 2);
  glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 0);
  glfwWindowHint(GLFW_OPENGL_DEBUG_CONTEXT, GL_TRUE);

  printf("Context defined\n");
  ...
  // initialise extension loader library GLAD
  gladLoadGL(glfwGetProcAddress);

  printf("Checking context flags\n");
  ...
  if (flags & GL_CONTEXT_FLAG_DEBUG_BIT) {
    ...
    glDebugMessageCallback(glDebugOutput_callback, NULL);
    glDebugMessageControl(GL_DONT_CARE, GL_DONT_CARE, GL_DONT_CARE, 0, NULL, GL_TRUE);
    printf("Debugging enbaled\n");
  }

  // Set buffer swap interval to 1 per frame
  glfwSwapInterval(1);
